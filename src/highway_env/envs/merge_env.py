...
# src配下のカスタム環境を使用するため、パスを調整
# ライブラリのhighway_envと区別するため、srcディレクトリをパスに追加
current_dir = os.path.dirname(os.path.abspath(__file__))
src_dir = os.path.dirname(os.path.dirname(current_dir))
if src_dir not in sys.path:
    sys.path.insert(0, src_dir)

# カスタム環境のモジュールをインポート（src配下のhighway_envを優先）
# まずsrc配下のモジュールをインポート
try:
    # 相対インポートを試みる
    from .common.abstract import AbstractEnv
    from ..road.lane import LineType, SineLane, StraightLane
    from ..road.road import Road, RoadNetwork
    from ..vehicle.controller import ControlledVehicle
    from ..vehicle.objects import Obstacle
    from .. import utils
except ImportError:
    # 相対インポートが失敗した場合は絶対インポートを使用
    # この場合、srcディレクトリがパスに追加されているため、src/highway_envが優先される
